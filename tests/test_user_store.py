...
class TestUserStore(unittest.TestCase):
    ...
    def test_touch_user_creates_record(self):
        user = self._fake_user()
        self.mod.touch_user(user, increment_requests=True)
        details = self.mod.get_user_details(1)
        self.assertIsNotNone(details)
        self.assertEqual(details["requests_count"], 1)

    def test_touch_user_increments_requests(self):
        user = self._fake_user()
        self.mod.touch_user(user, increment_requests=True)
        self.mod.touch_user(user, increment_requests=True)
        details = self.mod.get_user_details(1)
    ...
    def test_log_user_event(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self.mod.log_user_event(1, "test_event", {"key": "value"})
        details = self.mod.get_user_details(1)
        self.assertEqual(len(details["recent_events"]), 1)
        self.assertEqual(details["recent_events"][0]["event_type"], "test_event")

    def test_record_analysis_result_success(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self.mod.record_analysis_result(1, discipline="футбол", match_text="A vs B", success=True)
        details = self.mod.get_user_details(1)
        self.assertEqual(details["analyses_count"], 1)
        self.assertEqual(details["successful_analyses"], 1)

    def test_record_analysis_result_failure(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self.mod.record_analysis_result(1, discipline="футбол", match_text="A vs B", success=False)
        details = self.mod.get_user_details(1)
        self.assertEqual(details["analyses_count"], 1)
    ...
    def test_get_stats_summary(self):
        user = self._fake_user()
        self.mod.touch_user(user, increment_requests=True)
        stats = self.mod.get_stats_summary()
        self.assertEqual(stats["total_users"], 1)
        self.assertGreaterEqual(stats["total_requests"], 1)

    def test_list_recent_users(self):
        self.mod.touch_user(self._fake_user(1))
        self.mod.touch_user(self._fake_user(2))
        users = self.mod.list_recent_users(limit=10)
        self.assertEqual(len(users), 2)

    def test_get_user_details_not_found(self):
        result = self.mod.get_user_details(9999)
        self.assertIsNone(result)

    def test_touch_user_with_discipline(self):
        user = self._fake_user()
        self.mod.touch_user(user, discipline="хоккей", match_text="A vs B")
        details = self.mod.get_user_details(1)
    ...
    def test_daily_limit_fresh_user(self, mock_now):
        mock_now.return_value = datetime(2026, 4, 9, 10, 0, 0, tzinfo=MSK_TZ)
        self.assertTrue(self.mod.check_daily_limit(999, max_free=3))

    @patch('services.user_store.get_msk_now')
    def test_daily_limit_increment_and_reset(self, mock_now):
        mock_now.return_value = datetime(2026, 4, 9, 10, 0, 0, tzinfo=MSK_TZ)
        user = self._fake_user(1)
        self.mod.touch_user(user)

        self.mod.increment_daily_request(1)
        self.mod.increment_daily_request(1)
        self.mod.increment_daily_request(1)
        
        # 3 requests, limit is 3 => False
        self.assertFalse(self.mod.check_daily_limit(1, max_free=3))

        # Next day
        mock_now.return_value = datetime(2026, 4, 10, 10, 0, 0, tzinfo=MSK_TZ)
        self.assertTrue(self.mod.check_daily_limit(1, max_free=3))
    ...
    def _insert_promo(self, code="TESTCODE", active=True, max_uses=10, uses=0, days=30, requests=5):
        conn = self.mod.get_connection()
        cursor = conn.cursor()
        cursor.execute(
            self.mod._q(
    ...
    def test_activate_promo_valid_code(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self._insert_promo("FREE30", active=True, max_uses=10, uses=0, days=30, requests=5)
        result = self.mod.activate_promo(1, "FREE30")
        self.assertTrue(result["ok"])
        self.assertIn("активирован", result["message"])

    def test_activate_promo_invalid_code(self):
        result = self.mod.activate_promo(1, "NONEXISTENT")
        self.assertFalse(result["ok"])
        self.assertIn("не найден", result["message"])

    def test_activate_promo_inactive_code(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self._insert_promo("DEAD", active=False)
        result = self.mod.activate_promo(1, "DEAD")
        self.assertFalse(result["ok"])
        self.assertIn("неактивен", result["message"])

    def test_activate_promo_exhausted_code(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self._insert_promo("USED", active=True, max_uses=1, uses=1)
        result = self.mod.activate_promo(1, "USED")
        self.assertFalse(result["ok"])
        self.assertIn("исчерпан", result["message"])

    # --- check_user_access tests ---

    def test_check_user_access_free_user_within_limit(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        result = self.mod.check_user_access(1, max_free=3)
        self.assertTrue(result["allowed"])

    def test_check_user_access_premium_user(self):
        user = self._fake_user()
        self.mod.touch_user(user)
        self.mod.activate_subscription(1, days=30)
        result = self.mod.check_user_access(1, max_free=3)
        self.assertTrue(result["allowed"])
    ...
    def test_deactivate_expired_subscriptions(self):
        ...
        conn.commit()
        conn.close()
        count = self.mod.deactivate_expired_subscriptions()
