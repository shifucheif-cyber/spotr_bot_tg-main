...
class DataFetcherCacheTests(unittest.TestCase):
    ...
    def test_get_cached_expires_old_entry(self):
        ...
        self.assertIsNone(data_fetcher._get_cached(key))
    ...
    def test_phase_early_uses_7d_ttl(self):
        data_fetcher._put_cache("early_k", "data")
        ...
        self.assertEqual(data_fetcher._get_cached("early_k", phase=EventPhase.EARLY), "data")

    def test_phase_live_always_fresh(self):
        data_fetcher._put_cache("live_k", "data")
        data_fetcher._match_cache["live_k"]["ts"] = datetime.now(tz=timezone.utc) - timedelta(seconds=1)
        self.assertIsNone(data_fetcher._get_cached("live_k", phase=EventPhase.LIVE))

    def test_phase_finished_uses_48h_ttl(self):
        data_fetcher._put_cache("fin_k", "data")
        data_fetcher._match_cache["fin_k"]["ts"] = datetime.now(tz=timezone.utc) - timedelta(hours=47)
        self.assertEqual(data_fetcher._get_cached("fin_k", phase=EventPhase.FINISHED), "data")

    # --- Cleanup ---

    def test_cleanup_expired_cache(self):
        data_fetcher._put_cache("fresh", "data")
        data_fetcher._match_cache["old"] = {
            "result": "old-data",
            "ts": datetime.now(tz=timezone.utc) - timedelta(hours=49),
        }
        removed = data_fetcher.cleanup_expired_cache()
