...
ФИНАЛЬНЫЙ ОБЯЗАТЕЛЬНЫЙ FORMAT ОТВЕТА:
...
def format_response_contract(match_text: str, raw_analysis: str, prediction_struct: dict) -> str:
    ...
    winner = _extract_contract_field(raw_analysis, [r"Победитель:\s*(.+)", r"Исход:\s*(.+)", r"Прогноз победителя:\s*(.+)"])
    total = _extract_contract_field(raw_analysis, [r"Общий тотал:\s*(.+)", r"Тотал карт:\s*(.+)", r"Тотал:\s*(.+)"])
    score = _extract_contract_field(raw_analysis, [r"Прогноз по счету:\s*(.+)", r"Прогноз по сч[её]ту / картам / сетам:\s*(.+)", r"Сч[её]т:\s*(.+)"])
    
    prob_text = f"{prob:.0f}%" if prob is not None else "не определена"
    stake_text = f"{stake}%" if stake is not None else "ПРОПУСК"
