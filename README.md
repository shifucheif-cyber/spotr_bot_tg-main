...
### Автоматическая настройка Ubuntu (рекомендуется)

Одна команда — полная настройка сервера (PostgreSQL, venv, pip, .env):
...
#### Ubuntu 22.04 — системные зависимости
...
```bash
sudo -u postgres createuser --pwprompt spotr_bot
sudo -u postgres createdb -O spotr_bot spotr_bot_db
```
